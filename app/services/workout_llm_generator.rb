            ...
            sections: {
              ...
              items: {
                ...
                properties: {
                  ...
                  period_mins:        { type: "integer", description: "For format: emom only. Minutes per work block. Default 1 (a vanilla EMOM). Use 2 for E2MOM when the prescribed work won't fit in 1 minute (e.g. 5 Press Ups + 10 Slams + 20 Sit Ups every 2 min) or for heavy strength sets that need recovery; 3+ for very heavy or very long work blocks. The total `duration_mins` must be a multiple of `period_mins`." },
                  alternating: { type: "boolean", description: "For format: emom only. When true, exercises rotate one-per-period across the duration (M1=A, M2=B, M3=A, M4=B, ... at period_mins=1; or 2min Burpees / 2min Lunges / 2min Sit Ups / repeat at period_mins=2). When false, ALL exercises in the list are performed within each period. Default false (which means 'all together each period'). Irrelevant for single-exercise EMOMs. Example: 3 exercises with period_mins:1 + alternating:true = 'EMOM 15min · 3-min rotation, one exercise per minute' (Burpees → Lunges → Sit Ups → loop). Example: 3 exercises with period_mins:2 + alternating:false = 'E2MOM 20min · all 3 exercises every 2 minutes'." },
                  exercises: {
                    type: "array",
                    items: {
                      type: "object",
