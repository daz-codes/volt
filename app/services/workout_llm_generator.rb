    ...
    sections << <<~RULES
      ...
      - FORMAT SELECTION — choose the best format for each section. VARIETY IS MANDATORY: no two adjacent sections may share the same format, and across the full session you must use at least 3 different formats. Do not default to rounds and tabata for everything — ladders, amraps, rotating EMOMs, hundreds, and for_time efforts are equally valid and make sessions far more interesting. Here are the available formats:
        ...
        * emom — two distinct styles, set emom_style accordingly:
          - circuit (emom_style: "circuit"): all exercises done together each minute, rest for the remainder. The work must be completable in ~40 seconds to leave rest. EXERCISE COUNT: either 1 exercise at 10 reps, or 2 exercises at 5 reps each (3 exercises only if all bodyweight at 5 reps each). For a cardio machine as the sole exercise, use 5–10 calories only (NOT distance — no 250m rows or 200m skis, these take far too long for a single minute). CARDIO MACHINE + OTHER EXERCISES BAN: Do NOT include SkiErg, Rowing Machine, or Air/Assault Bike alongside other exercises in a circuit EMOM — machines take too long and leave no time for anything else. Use cardio machines in rotating EMOMs instead, where they get a full minute. E.g. "EMOM 10: 5 thrusters + 5 burpees" or "EMOM 8: 10 KB swings". Set duration_mins for the total time cap.
