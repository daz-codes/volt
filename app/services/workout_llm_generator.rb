...
class WorkoutLLMGenerator
            ...
            sections: {
              ...
              items: {
                ...
                properties: {
                  ...
                  format:             { type: "string", enum: %w[straight amrap rounds emom continuous_circuit tabata for_time ladder mountain matrix hundred switchback], description: "straight=ONE continuous block, NO round repetition. Use for warm-up, cool-down, the main long-cardio block at low intensity (e.g. one 25-min zone 2 run as a single block — `format: straight, duration_mins: 25`), and mobility flow sections that list multiple drills each with their own `duration_s` (e.g. Hip Mobility flow: 90/90 90s + Couch Stretch 120s + Hip CARs 90s). NEVER set a `rounds:` value on a `straight` section. If you find yourself wanting to repeat the same exercise multiple times, the format is `rounds`, not `straight`, rounds=multiple rounds of the same set — rep-based (each exercise has reps) OR duration-based (a single movement with duration_s replaces reps for time-interval finishers like 3 rounds: 2min Wall Balls / 2min rest), amrap=as many rounds as possible in a time cap, emom=every N minutes on the minute (N is `period_mins`, default 1). Set `period_mins` based on whether the prescribed work fits in 1 minute — if it doesn't (e.g. 5 Press Ups + 10 Slams + 20 Sit Ups won't fit in 60s), use `period_mins: 2` (E2MOM); for heavy strength sets that need recovery use 2-3. Set `alternating: true` when exercises rotate one-per-period (M1=A, M2=B, M3=A...), `alternating: false` when ALL exercises happen within each period. SINGLE-exercise EMOMs are typically `period_mins: 1` with the '~50% of 1-min max' cue in notes and blank reps. continuous_circuit, continuous_circuit=cycle one exercise per minute through the list (duration_mins must be a multiple of exercise count), tabata=20s work/10s rest×8, for_time=complete prescribed reps/distance as fast as possible (record finishing time), ladder/mountain=reps/distance change each round, matrix=progressive exercise combination (add then remove exercises each round: A → A+B → A+B+C → B+C → C), hundred=100 reps of a single exercise for time (The Centurion), switchback=Up & Down Ladder pairing cardio (calories) with a functional movement (reps) where the cardio counts down and the functional counts up — exactly 2 exercises, set start/end/step fields on the section" },
                  duration_mins:      { type: "integer" },
                  rounds:             { type: "integer" },
                  rest_secs:          { type: "integer", description: "Rest in seconds after each round. 30/45/60 for low and medium intensity sections; 90/120/180 for high intensity sections (heavy lifts and sprint intervals need long rest)." },
                  intensity_style:    { type: "string", enum: %w[low medium high], description: "Training intensity for this section. low=conversational pace (RPE 4-5) — LONG continuous cardio (20+ min straight blocks, or 10-min stacks across row/run/ski), bodyweight mobility/technique only, NO weights, NO EMOMs, NO Tabatas, NO sprints, duration_mins must be a multiple of 5. medium=working pace (RPE 7-8), ~10 reps with last 2 hard, 60s rest between rounds — the default for most metcon-style work. high=near-max (RPE 9-10), strength block goes FIRST after warm-up, low reps heavy OR sprint intervals, 120-180s rest, fewer total sections to allow unhurried transitions; prefer EMOMs over continuous_circuit for metcons. Optional — omit for mixed sessions where each section's style is implicit." },
                  ...
                  period_mins:        { type: "integer", description: "For format: emom only. Minutes per work block. Default 1 (a vanilla EMOM). Use 2 for E2MOM when the prescribed work won't fit in 1 minute — the canonical E2MOM has 2-3 exercises performed ALL TOGETHER inside each 2-min window with explicit reps (e.g. 5 Press Ups + 10 Slams + 20 Sit Ups every 2 min). The total `duration_mins` must be a multiple of `period_mins`. When period_mins > 1, DO NOT set alternating:true (that combination has no meaningful training shape) and DO NOT use the '~50% of 1-min max' cue (it's built for 1-minute blocks). Set explicit reps/distance/calories on each exercise instead." },
                  alternating: { type: "boolean", description: "For format: emom only — and ONLY valid at period_mins:1. When true, exercises rotate one-per-period across the duration (M1=A, M2=B, M3=A, M4=B, ... — exactly one exercise per minute, cycling). Use for rotating-EMOM patterns at period_mins:1. **DO NOT set alternating:true with period_mins > 1** — that combination has no real training utility (it's just a slower rotation, achievable more cleanly with period_mins:1). For E2MOM and longer periods, leave alternating unset/false: the canonical E2MOM is 2-3 exercises ALL performed within each 2-min window (e.g. 5 Press Ups + 10 Slams + 20 Sit Ups every 2 min, with explicit reps — the 1-min-max cue does NOT apply here). Default false. Irrelevant for single-exercise EMOMs." },
                  exercises: {
                    type: "array",
                    items: {
                      type: "object",
                      ...
                      properties: {
                        ...
                        notes:       { type: "string", description: "Form cues and intensity guidance ONLY (e.g. 'explosive hip drive', 'slow tempo', 'moderate weight'). NEVER put sets, rounds, reps, distances, weights, or programming details here — use the structure fields instead. Athlete-relative effort cues that reference rep targets are allowed (e.g. '~50% of your 1-min max'). The 'never put reps in notes' rule applies to numeric rep counts that duplicate the reps field — not to athlete-calibrated cues." },
  ...
  def initialize(user:, duration_mins:, activity: nil, group_tag_name: nil, source_workout: nil, session_notes: nil, equipment: nil, injury_notes: nil, intensity_style: nil, **_legacy)
    ...
    @intensity_style = intensity_style.to_s.presence_in(%w[low medium high])
